...
an entry of the catalog that already exists: `okta-workforce` in
`tenants/azure/corp/entra-enterprise-apps/saml-apps.hcl`, a custom SAML
application whose identifier and reply URL are the Okta side's audience and
ACS URL (ADR 0021). Nothing new was built on the Entra side, because a
service provider is what that catalog onboards, and Okta is one.
...
signs the assertion by default, so the cells set `ASSERTION`); how the
asserted subject is matched to an Okta user (a match type from an
allowlist, the NameID formats accepted, an optional filter, the username
...
an HTTP-POST binding; none of these has an input. The routing target is
...
form from the Entra portal or read from the `value` of the Entra module's
signing certificate resource, and the fragment passes it with
`file("${get_terragrunt_dir()}/entra-signing-<year>.cer")`, so the text is
read from beside the cell rather than pasted into it. ADR 0017 is amended
for exactly that: a `.cer` beside a cell, referenced by `file()` with
`get_terragrunt_dir()`, is the only non-`.hcl` file a cell directory holds
and the only function call an input may carry. The module strips the
...
before prod relies on it; the two cells differ in that one line.
...
- **Rotation is coordinated, not zero-downtime by itself.** Okta trusts
  ...
  because the provider documents the key resource as create-or-remove and
  a key the identity provider still references cannot be deleted. The
  `identity_providers` output shows both keys with their expiry while a
  rotation is in progress.
- **The Entra side carries values Okta mints.** The `okta-workforce`
  ...
  comment on the entry says so. That is the one place in the estate where
...
- **The cell rule has one stated exception, and it is narrow.** A cell
  ...
  `terragrunt.hcl` with no change. A cell that needs any other file or
  function has found a shape the catalog does not offer.
- **OIDC and social identity providers, the inverse direction, and the
  ...
  different trust in the other tree; and the `okta-workforce` application's
  OAuth-consented provisioning connector is a person in a dialog, as ADR
  0021 says of Google Workspace's.
- **What a first apply should confirm.** That the org has the
  ...
  `thumbprint` in the `identity_providers` output equals the thumbprint the
  Entra cell reports for the same certificate, since a mismatch means the
  wrong file crossed; that a workforce sign-in routes through Entra and
