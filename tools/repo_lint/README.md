...
- `repo_lint.py` checks the tree against eight rules the README and the
...
- `cell-shape`: `forbidden-block`, `missing-include`, `missing-terraform`,
  ...
  `dependency-outside-tenants`, `dependency-missing`.
...
template ids, and first-party application ids. Every line is the id followed
...
instead of restating it. `.github/workflows/aws-release.yml` reads it, in a
...
|--------|---------------|---------------------------|
| `cell-files` | a file inside the cell's own directory changed | the cell's values are the plan's inputs |
...
catalog, and an app cell, a subscription with the same, a stack that
...
secret-looking strings: one cell per `cell-shape` code, a locator per
...
The `ascii` and `no-secrets` failures, and the one cell that cannot be
parsed, are written into a copy of the good tree at test time, from
