output "identity_providers" {
  description = "Map of logical key to { id, name, status, audience, acs_type, active_certificate, kid, keys }. audience is the SP entity id Okta computed for the trust; kid is the key in use; keys maps every certificate name to { kid, x5t_s256, expires_at }. Nothing here is secret: a kid is a reference and a thumbprint is a hash of a public certificate."
  value = {
    for k, p in okta_idp_saml.this : k => {
      ...
      keys = {
        for cert_name in keys(var.identity_providers[k].signing_certificates) : cert_name => {
          kid        = okta_idp_saml_key.this["${k}/${cert_name}"].kid
          x5t_s256   = okta_idp_saml_key.this["${k}/${cert_name}"].x5t_s256
          expires_at = okta_idp_saml_key.this["${k}/${cert_name}"].expires_at
        }
      }
    }
  }
